...
var UserPermission = require('permission/models/user_permission.model'); 
...
var permissionRepository = {
   ...
   getPermissionByUser: async (userId) => {
    try {
            let permResult = await Permission.aggregate([
                {
                    "$project": {
            ...
            let permissions = async () => {
                await utils.asyncForEach(permResult, async perm => {
                    ...
                    await utils.asyncForEach(perm.permission_list, async innerPerms => {
                        let permId = innerPerms._id;
                        let userInfo = await UserPermission.findOne({$and:[{'userId':userId},{'permissionall': {$in: [permId]}}]});
                        innerPerms.hasAccess = (userInfo != null)?true:false;
                        // perm['hasAccess'] = (roleInfo != null)?true:false;
                        // console.log(32, innerPerms.hasAccess);
                        permissionLists.push(innerPerms);
                        perm.permission_list = permissionLists;
    ...
    updateUserPermissionById: async (field,data) => {
        try {
            let update = await UserPermission.findOneAndUpdate(field, data,{upsert: true, 'new': true});
            if(!update) {
                return null;
            }
            return update;
